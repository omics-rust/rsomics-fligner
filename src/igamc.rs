...
//!
//! scipy computes the Fligner-Killeen p-value as `chi2.sf(statistic, k-1)`,
//! which inside SciPy is `cephes.chdtrc(df, x) = igamc(df/2, x/2)` — the
//! regularized upper incomplete gamma. We port Cephes `igam`/`igamc` directly
//! so the tail matches SciPy's special-function path bit-for-bit.
...
mod tests {
    ...
    fn chi2_sf_matches_scipy() {
        // scipy.stats.chi2.sf(x, df)
        close(
            chi2_sf(2.0, 7.584_952_754_501_659),
            0.022_539_715_842_708_703,
            1e-12,
        );
        close(chi2_sf(2.0, 1.0), 0.606_530_659_712_633_4, 1e-12);
        close(
            chi2_sf(1.0, 3.841_458_820_694_124),
            0.049_999_999_999_999_94,
            1e-12,
        );
    }
}
